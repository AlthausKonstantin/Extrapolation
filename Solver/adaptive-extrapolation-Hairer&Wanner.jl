...
"""
    AdaptiveExtrapolationHW

This module is  implementing an
adaptive extrapolation of the explicit midpoint rule according to Hairer and Wanner.
"""
module AdaptiveExtrapolationHW
...
The algorithm adaptively controls stepsize and extrapolation order in the line of thought of Hairer and Wanner.
...
* `parameterOrderSelection::S`. Change the current order `n` to order `m` if `work[m] < parameterOrderSelection⋅work[n]`. Default is `0.9`.
...
struct solver
   ...
   # weights and scaling factors for internal extrapolation operators (cf. 2.16)
   ...
   function solver(N::Integer, sequence::String)
       ...
       if sequence == "Harmonic"
           ...
           #nodes = [BigInt(1)//(n+1)^2 for n = 0:N]
       ...
       ω = ones(Rational{BigInt},N+1,N+1)
       for n = 1:N
...
function (mySolver::solver)(
    ...
    if 2<= minimumOrder <= maximumOrder <= length(mySolver.ρ)-1
        n_ex, N_ex = minimumOrder, maximumOrder
    else
        error("Minimal and maximal order must satisfy: 2 ≦ minimumOrder ≦ maximumOrder ≦ $(length(mySolver.ρ)-1)!")
    end
    ...
    if parameterOrderSelection > 0
        ρ = parameterOrderSelection
    else
        error("parameterOrderSelection must be positive!")
    end
    ...
    μ = zeros(S, N_ex+1) # μ[k] is the stepsize scaling factor for extr. order (k -1)
    τ_opt = zeros(S, N_ex+1) # τ_opt[k] is the optimal stepsize for extr. order (k -1)
    w = zeros(S,N_ex+1)# w[k] is the work of the current step for extr. order (k -1)
    s = [2*sum(Int64.(mySolver.subdividingSequence[1:n+1])) - n for n in 0:N_ex] # s[k] is the number of stages for  extrapolation order (k-1)
    ...
    while !finished
        ...
        m = max(n_ex+1,min(N_ex-1,m))
        n_win, N_win = m-1, m+1 # order window, n_ex+1 ≦ m ≦ N_ex-1 is enforced below
        ...
        #second compute all information relating to an order ≦ n_win:
        ...
        for i = n_win - 1 : n_win
            ...
            μ[i+1] = min(p2, max(p1, (tol/ϵ_i)^(1/(2i+1)))) # factor for optimal stepsize. including safety scaling
            if i == n
        ...
        while n <= N_win
            if ϵ_n <= tol
                ...
                converged = true
                break
            elseif (m < n_ex + 1) || ϵ_n <= tol*S(prod(mySolver.subdividingSequence[n+2:N_win+1].//mySolver.subdividingSequence[1])^2)
                ...
                μ[n+1] = min(p2, max(p1, (tol/ϵ_n)^(1/(2n+1)))) # factor for optimal stepsize, including safety scaling
            else
        ...
        if converged
            ...
            if abs(tRest) < eps(S)
                ...
                ## compute optimal order m and stepsize τ for the next step acc.Hairer Wanner
                temp = (n_win:n+1) # index range of computed quantities
                τ_opt[temp] = min.(abs(τ)*μ[temp], maximalSteplength) # safety boundary for τ_opt
                w[temp] = s[temp]./(τ_opt[temp]) # work per step

                # order selection
                if n == n_ex # enforce n_ex + 1 ≦ m
                    m = n + 1
                elseif counterReduction != 0 # do not increase order after reduction
                    m = min(m,n)
                else
                    if n <= m
                        if w[n-1] < ρ*w[n]
                            m = max(n-1,n_ex + 1)
                        elseif w[n] < ρ*w[n-1]
                            m = min(n+1,N_ex-1) # <= m+1 since n <=m
                        else
                            m = n # >=n_ex+1 and <= N_ex+1 since n<=m <= N_ex+1
                        end
                    else
                        if w[m] < ρ w[m+1]
                            m = max(m-1, n_ex +1)
                ...
                # stepsize selection
                if m == n + 1
                    # compute optimal stepsize of order m based on the optimal stepsize of n
                    τ_opt[m+1] = min(τ_opt[n+1]*s[n+2]/s[n+1], maximalSteplength)
                end
                τ  = flipsign(τ_opt[m+1],τ)
            ...
            # compute reduced order m and stepsize τ
            if n == m - 1
                m = max(n_ex+1,m-1)
            end
            μ[m+1] = min(p2, max(p1,μ[m+1]))
            τ = flipsign(min(abs(τ)*μ[m+1], maximalSteplength), τ)
        end
...
export solver
end # end module AdaptiveExtrapolationHW
